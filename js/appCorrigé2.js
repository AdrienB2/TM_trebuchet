...
const tmax = 10000; // Temps maximal en seconde que la simulation peut durer (But: éviter un temps de calcul trop long)
...
function calculsAccAngu(alpha, beta, gamma, d1alpha, d1beta, d1gamma) {
    ...
    // alpha = angle bras gauche (l2)-fronde (l5)
    // beta  = angle mat vertical (l3)-bras gauche (l2)
    ...
    d2alpha = I[0][0] * w1 + I[0][1] * w2 + I[0][2] * w3; //w1*(I[0][0]+I[1][0]+I[2][0])
    d2beta = I[1][0] * w1 + I[1][1] * w2 + I[1][2] * w3; //w2*(I[0][1]+I[1][1]+I[2][1])
    d2gamma = I[2][0] * w1 + I[2][1] * w2 + I[2][2] * w3; //w3*(I[0][2]+I[1][2]+I[2][2])
    ...
    for(let a=0;a<=i;a++){
        if(a <= i_release){
            ...
            if(a <= i_continue){
                ...
                Energies.push(ETreb[ETreb.length-1] + (1 / 2) * m2 * vitesses[a] ** 2 + m2 * g * positionsy[a]);
            };
        }
    }
}
    ...
    animInterval = setInterval(() => {
        ...
        if (i < positionsx.length) {
            ...
            if (i < i_release || focusMode) {
                ...
                draw(currentAlpha,currentBeta,currentgamma,{x: positionsx[i], y: positionsy[i]},vitesses[i],Energies[i],i,currentScale,0);
            } else {
