...
const rendementCollision = 0.25; //rendement des collisions BOIS vs BOIS
...
function calculsAccAngu(alpha, beta, gamma, d1alpha, d1beta, d1gamma) {
    ...
    // alpha = angle entre bras gauche (l2) et fronde (l5)
    // beta  = angle entre mat vertical (l3) et bras gauche (l2)
    // gamma = angle entre bras droit (l1) et longueur du contrepoids (l4)
...
function EulerMethod(dt, presenceBalle) {
    ...
    //approximation du second ordre avec Euler : x = x0 + dx/dt * dt + 1/2 * d^2x/dt^2 * dt^2
    ...
    //vitesse angulaires de beta et gamma après les collisions
...
function balistique(vi, incl, xi, yi) {
    ...
    function fA(vol) { //force d'Archimède 
        return rho * vol * g;
    }

    function a(vx, vy) { //accélération du projectile balistique
        return {
...
function draw(alpha, beta, gamma, pos, vitesse, Energy, i, scale) {
    ...
    if (l2 + l5 + r2 > l1 + l4 + r1) {
        var c = {
            x: scale * (-xmin * 1.1 + l2 + l5 + r2),
            y: simulation.height - scale * l3,
        };
    } else {
        var c = {
            x: scale * (-xmin * 1.1 + l1 + l4 + r1),
            y: simulation.height - scale * l3,
        };
    }
    var b = {
    ...
    if (animPoints.length > 0) {
        for (let k = 0; k < animPoints.length; k++) {
            ...
            ctx.arc(
                ...
                simulation.height - animPoints[k].y * scale,
                0.8,
                0,
                2 * PI
            );
    ...
    if (i >= positionsx.length - 1) {
        for (var k = 0; k < positionsx.length;) {
            ...
            ctx.arc(
                ...
                simulation.height - positionsy[k] * scale,
                1.2,
                0,
                2 * PI
            );
...
function calculate() {
    ...
    vitesses.push((l2 ** 2 * d1alpha ** 2 + l5 ** 2 * (d1beta - d1alpha) ** 2 + 
                   2 * l2 * l5 * d1alpha * (d1beta - d1alpha) * Math.cos(beta)) ** 0.5
    );
    ...
    let E = (1 / 2) * (m1 * (l1 ** 2 + l4 ** 2) + 
    mb * lb ** 2 - 
    2 * m1 * l1 * l4 * Math.cos(gamma) + 
    m2 * (l2 ** 2 + l5 ** 2) - 
    2 * m2 * l2 * l5 * Math.cos(beta)) * d1alpha ** 2 + 
    (1 / 2) * m1 * l4 ** 2 * d1gamma ** 2 + 
    (1 / 2) * m2 * l5 ** 2 * d1beta ** 2 + 
    m1 * (l4 ** 2 - l1 * l4 * Math.cos(gamma)) * d1alpha * d1gamma + 
    m2 * (-(l5 ** 2) + l2 * l5 * Math.cos(beta)) * d1alpha * d1beta + 
    g * ((m1 * (massV_weightLenth - masseVolFluide)) / massV_weightLenth + 
    (m2 * (massV_ball - masseVolFluide)) / massV_ball + 
    (mb * (massV_bois - masseVolFluide)) / massV_bois) * l3 + 
    g * (((-m1 * (massV_weightLenth - masseVolFluide)) / massV_weightLenth) * l1 + 
    ((m2 * (massV_ball - masseVolFluide)) / massV_ball) * l2 + 
    ((mb * (massV_bois - masseVolFluide)) / massV_bois) * lb) * Math.cos(alpha) - 
    ((g * (massV_ball - masseVolFluide)) / massV_ball) * m2 * l5 * Math.cos(beta - alpha) + 
    ((g * (massV_weightLenth - masseVolFluide)) / massV_weightLenth) * m1 * l4 * Math.cos(gamma + alpha);
    ...
    balistique(
        ...
        positionsx[i_release],
        positionsy[i_release]
    );
    ...
    while (i_continue < i && l3 + l2 * Math.cos(alpha) - l5 * Math.cos(beta - alpha) >= 0) {
        switch (methode) {
            case "RungeKutta":
                RungeKutta(simDT, false);
                break;
            case "Euler":
                EulerMethod(simDT, false);
                break;
            default:
                Heun(simDT, false);
        }
    ...
    animInterval = setInterval(() => {
        ...
        if (i < positionsx.length) {
            ...
            if (i < i_release || focusMode) {
                ...
                //sinon, on diminue le scale petit-à-petit jusqu'au scale qui affiche toute la simulation
